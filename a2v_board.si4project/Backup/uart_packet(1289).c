...
void uart_packet_Exec(int cmd_num, uint8_t *tuningA2V, uint8_t info1, uint8_t info2)
{
	switch(cmd_num)
	{
		case CMD_CHK_CONN:
			...
			break;
		case CMD_SET_TUNE:
			tuningA2V[info1] = info2;
			uart_packet_Response(cmd_num, info1, tuningA2V[info1]);
			break;
		...
		case CMD_REQ_DBG:
			uart_packet_Debug(cmd_num, tuningA2V, info1, info2);
			break;

		case CMD_REC_TUNE:
			util_flash_tuning_write(tuningA2V + info1, info2);
			uart_packet_Record(cmd_num, tuningA2V, info1, info2);
			break;

		case CMD_REG_WRT:
			dw7914_set_byte(info1, info2);
			uart_packet_Register(cmd_num, info1, info2);
			break;

		case CMD_REC_WAVE:
			util_waveform_to_flash();
			uart_packet_Response(cmd_num, info1, info2);
			break;
...
void uart_packet_Decode(char *packet, uint8_t *tuningA2V)
{
	...
	if(packet[cnt++] == '$') {
		...
		info1 = asc2hex(packet[cnt++]) * 16;
		info1 += asc2hex(packet[cnt++]);
		info2 = asc2hex(packet[cnt++]) * 16;
		info2 += asc2hex(packet[cnt++]);
		if(packet[cnt++] == '#') {
			uart_packet_Exec(cmd_num, tuningA2V, info1, info2);
		}
