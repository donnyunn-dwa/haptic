...
void dw7914_read_RAM(uint8_t* pData, uint16_t addr, uint16_t len)
{
	...
	i2c_write(DW7914_RAM_ADDRH, buf, 2);

	i2c_read(DW7914_RAM_DATA, pData, len);
}
...
void dw7914_write_RAM(uint8_t* pData, uint16_t addr, uint16_t len)
{
	...
	i2c_write(DW7914_RAM_DATA, pData, len);
}
...
{	
	...
	// Initial Check
	if(dw7914_get_byte(DW7914_CHIP_ID) != haptic->chip_id) {
		Error_Handler();
	}

	dw7914_get_byte(DW7914_STATUS0);

	// Initial Setup
	haptic->mode = MODE_MEM_PLAYBACK;
	dw7914_set_byte(DW7914_MODE, haptic->mode);

	haptic->pwm_freq = PWM_FREQ_PWM_FREQ_48KHZ;
	dw7914_set_byte(DW7914_PWM_FREQ, haptic->pwm_freq);

	haptic->boost_mode = BOOST_MODE_BST_ADAPT;
	dw7914_set_byte(DW7914_BOOST_MODE, haptic->boost_mode);

	haptic->vd_clamp = VD_CLAMP_1000;
	dw7914_set_byte(DW7914_VD_CLAMP, haptic->vd_clamp);
