...
#define HYST 10
void vibFreqGenSet(parameterA2V *dw)
{
	int loudness_cmp;

	loudness_cmp = dw->loudnessOut_R - dw->loudnessOut_Base;
	...
	if(runFlag) {
		...
		if(loudness_cmp > (dw->in_loudnessThres + HYST)) {
			...
			if(loudness_cmp > 0) {
				dw->loudnessOut_Base = (15*dw->loudnessOut_Base + dw->loudnessOut_R) / 16;
			} else {
				dw->loudnessOut_Base = dw->loudnessOut_R;
			}
		}
	}
}
...
int DW_A2V_Ctrl(parameterA2V *dw, short* audioInLib,unsigned char *tuningA2V)
{
	...
	if(a2vEnable) {
		...
		dw->loudnessOut_R = loudnCalc(dw->duPow,dw);
		vibFreqGenSet(dw);													// set vibration amp & freq 
